...
export const OrderInfoStep = () => {
  ...
  const onSubmit = (data: FormType) => {
    if (!user && !USE_MOCK_API) return;

    if (!USE_MOCK_API && data.useAccountInfo && (!user?.phone || !user?.address)) {
      toastError(
        "Thông báo",
    ...
    const phone: string =
      !USE_MOCK_API && data.useAccountInfo ? user?.phone ?? "" : data.phone ?? "";

    const address: string =
      !USE_MOCK_API && data.useAccountInfo
        ? user?.address ?? ""
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        {useAccountInfo && user && !USE_MOCK_API ? (
          <AccountInfo
            phone={user.phone}
